...
export default function GlobalChatClient() {
  ...
  // initial load (latest) - uses SSE for real-time updates
  const { data } = useSWR<{ messages: Msg[] }>(
    '/api/chat/global/messages?limit=100',
    fetcher,
    {
  ...
  const handleNewMessage = useCallback(
    (message: Msg) => {
      ...
      // Auto-scroll if near bottom (within 120px of bottom)
      const container = containerRef.current;
      if (!container) return;
      const nearBottom =
  ...
  useEffect(() => {
    if (messages.length === 0) return; // Only scroll on first load
    scrollToBottom();
  }, [messages.length === 0, scrollToBottom]); // first load only
  ...
  async function send() {
    ...
    try {
      ...
      setMessages((prev) => {
        const newMessages = dedupeById([...prev, j.message as Msg]);
        // Scroll to bottom after state update with a small delay to ensure DOM update
  ...
  const parseMentions = (text: string) => {
    // Create a map of displayNames to users for quick lookup
    const displayNameMap = new Map<string, MentionUser>();
    mentionUsers.forEach((user) => {
      displayNameMap.set(user.displayName.toLowerCase(), user);
    });
    ...
    return urlParts.map((urlPart, urlIndex) => {
      ...
      // Split by mentions (@displayName) - single words only, max 16 chars, case-insensitive
      ...
      return mentionParts.map((mentionPart, mentionIndex) => {
        if (mentionPart.startsWith('@')) {
          const displayName = mentionPart.substring(1);
          const user = displayNameMap.get(displayName.toLowerCase());
          ...
          if (user) {
            return (
              <button
                ...
                onClick={() =>
                  handleUsernameClick(user.authUserId, user.displayName)
                }
